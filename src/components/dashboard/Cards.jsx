...
const Cards = () => {
  ...
  const resolvedToday = tickets.filter(
    (ticket) =>
      ticket.resolvedAt && new Date(ticket.resolvedAt).toDateString() === today,
  ).length;
  ...
  const activeAssigneeCount = tickets.filter((t) =>
    ["assigned", "in-progress", "reopened"].includes(
      (t.status || "").toLowerCase(),
    ),
  ).length;
  const slaRiskCount = tickets.filter((t) =>
    ["overdue", "due-soon"].includes(getTicketSlaState(t)),
  ).length;

  const sharedCards = [
    {
      id: 3,
      label: user?.role === ROLES.CLIENT ? "My Open Requests" : "Open Tickets",
      value: openCount,
    ...
    },
    {
      id: 4,
      label:
        user?.role === ROLES.CLIENT ? "Resolved Requests" : "Resolved Tickets",
      value: resolvedCount,
      colors:
  ...
  return (
    ...
    <div className="grid w-full grid-cols-1 items-center gap-3 border-b border-gray-200 pb-4 dark:border-gray-800 xs:grid-cols-2 sm:grid-cols-5 xl:grid-cols-6">
