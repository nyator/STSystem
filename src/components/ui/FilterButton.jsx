...
function FilterButton({
  ...
  return (
    <div className={`relative ${isOpen ? "z-50" : "z-0"}`} ref={ref}>
      ...
      {isOpen && (
        <div
          className="absolute right-0 top-full z-50 mt-2 max-h-96 w-52 overflow-y-auto rounded-lg border border-gray-200 bg-white p-2 shadow-xl shadow-gray-200/80 dark:border-gray-700 dark:bg-gray-800 dark:shadow-none"
          onMouseDown={(e) => e.stopPropagation()}
        >
          {hasActiveFilters && (
            <button
            ...
            >
              ...
              {group.options.map((option) => (
                <button
                  key={option.value}
                  onClick={() =>
                    handleOptionClick(group.filterType, option.value)
                  }
                  className={`z-10 w-full text-left text-nowrap flex items-center justify-between px-3 py-2 text-xs rounded-md transition-all ease-in-out duration-200 focus:outline-none ${
                    filters[group.filterType] === option.value
